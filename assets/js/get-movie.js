...
function formMovieTableData(movies){
    ...
    for(let movieObj of movies){
        content += `<tr><td>${i++}</td><td>${movieObj.title}</td><td>${movieObj.language}</td><td><img src="images/${movieObj.imageUrl}" width="50px" height=50px"></td><td><a href="edit-movie.html?id=${movieObj.id}">Edit</a> </td></tr>`;
    }
    console.log(content);
